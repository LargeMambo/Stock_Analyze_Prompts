...
**策略规则目录（如 `乌龟战法_v0.15/`）为只读目录，严禁在其中添加、修改或删除任何文件或子文件夹。**
...
{workspace}/
│
├── 乌龟战法_v0.15/            ← 策略规则目录（⚠️ 只读，禁止修改/添加）
│   ├── coordinator.md
│   ├── phase1_数据采集.md
│   ├── phase2_PDF解析.md
│   └── phase3_分析与报告.md
...
- `{strategy_dir}`：策略规则目录路径（如 `{workspace}/乌龟战法_v0.15/`）

**协调器在启动 Phase 1/Phase 2 前，必须先创建输出目录**：`mkdir -p {workspace}/{symbol}/`

---

*乌龟战法 v0.15 | 多阶段 Sub-agent 架构 | Coordinator*
