# 乌龟战法：AI选股分析Prompt v0.11

> **版本说明**：v0.11 — 基于v0.10，主要变更：
> 1. **report_template因子2/3/4章节新增「变量索引」**：在报告模板的因子2、因子3、因子4章节标题下方各加入变量速查表，列出该因子使用的全部变量符号、含义及首次出现位置，方便生成报告时快速对照
> 2. **因子3步骤10新增「收入敏感性分析」**：在修正穿透回报率计算后，新增收入下滑情景分析（1.0×/0.9×/0.8×/0.7×），通过经营杠杆系数估算各情景下的穿透回报率，并计算临界收入倍数（穿透回报率刚好跌破门槛的收入水平）
> 3. **report_template 5.10节同步更新**：新增敏感性分析表格输出格式
>
> v0.10变更保留：
...
*乌龟战法 AI Prompt v0.11 | 价值型 / 估值回归型选股 | 变量索引+收入敏感性分析增强版*
