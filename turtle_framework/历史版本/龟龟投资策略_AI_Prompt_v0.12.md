# 乌龟战法：AI选股分析Prompt v0.12

> **版本说明**：v0.12 — 基于v0.11，主要变更：
> 1. **因子2/3定位重构**：因子2重命名为「穿透回报率粗算（Top-Down）」，因子3重命名为「穿透回报率精算（Bottom-Up）+ 现金质量审计」，明确两者为粗算→精算递进关系
> 2. **因子2独立否决门槛**：新增R < Rf或R < II×0.5时直接否决、不进入因子3的快速筛除规则
> 3. **模块〇(8)参数预提取与变量标号对齐**：参数表直接使用A-HH变量标号，消除预提取与后续因子间的映射模糊
> 4. **知识库计算步骤统一变量标注**：所有因子2/3/4计算公式后附变量标号，格式为「归母净利润 = 集团净利润 − 少数股东损益（C = A − B）」
> 5. **广义vs狭义现金口径决策规则**：因子3步骤8新增明确的口径选择条件和升级规则
> 6. **外推可信度评级标准显式化**：新增5维度评分矩阵，替代原隐式判断
> 7. **report_template新增财务趋势速览表**：在因子1A之前插入近5年核心财务指标趋势总览
> 8. **放宽结论修改规则**：允许用户提供新信息后要求重新评估，禁止无理由修改
> 9. **税率表补充港股通场景**：H股港股通税率20%（vs直接持有28%），A股增加分档税率
>
> v0.11及之前变更保留：
...
第2步：母公司净现金 = 广义现金 - 有息负债（引用模块〇(8)C项）
...
本报告由AI模型基于乌龟战法v0.12框架和公开财务数据自动生成，仅供参考，不构成投资建议。投资者应自行核实所有数据，并根据自身风险承受能力做出投资决策。AI分析存在以下固有局限：(1) 财务数据可能存在提取误差；(2) 联网搜索的市场数据可能存在时滞；(3) 定性判断依赖有限信息；(4) 历史数据不代表未来表现。
```
...
*乌龟战法 AI Prompt v0.12 | 价值型 / 估值回归型选股 | 粗算精算分层+外推可信度评级+港股通美股支持*
