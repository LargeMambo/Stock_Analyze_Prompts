# 乌龟战法：AI选股分析Prompt v0.13

> **版本说明**：v0.13 — 基于v0.12，主要变更：
> 1. **模块九新增母公司单体负债分析**：SOTP估值中区分母公司层面负债与合并口径负债，避免子公司负债双重扣除；新增母公司单体资产负债表提取规则及不可获取时的降级处理
> 2. **因子4新增「股价位置与买入触发价评估」**：获取过去10年历史周线数据，计算当前股价历史分位；由门槛II%反推目标买入价，并计算目标价在历史价格中的分位，评估买入触发概率
>
> v0.12及之前变更保留：
> - 因子2/3定位重构（粗算Top-Down / 精算Bottom-Up）
> - 因子2独立否决门槛（R < Rf 或 R < II×0.5 → 直接否决）
> - 模块〇(8)参数预提取与变量标号对齐（A-HH）
> - 知识库计算步骤统一变量标注
> - 广义vs狭义现金口径决策规则
> - 外推可信度5维度评级标准
> - report_template财务趋势速览表
> - 放宽结论修改规则
> - 税率表港股通/A股分档/美股场景
> - 因子1B模块九（控股折价分析：SOTP、市值拆解、折价分解、敏感性）
> - 模块〇所有数据表含「数据来源」列及(8)参数预提取
> - 现金口径扩展（广义/狭义）
> - 模块三两层递进护城河结构
> - 因子4价值陷阱排查#2细化为商业层/技术层
> - report_template因子2/3/4变量索引
> - 因子3收入敏感性分析
...
1. **实时数据获取（强制执行）**：以下数据**必须通过外部数据源获取最新值**，严禁凭记忆推测：
   - 最新股价（收盘价）及对应日期
   - 最新总市值与总股本
   - 无风险利率（Rf）：A股/港股标的使用中国十年期国债收益率；美股标的使用美国十年期国债收益率
...
第2步：母公司层面净现金/净负债 ⭐ v0.13
  ...
  港股/A股上市公司年报通常在合并报表之后附有「母公司资产负债表」
  （港股：Company-level Balance Sheet；A股：母公司资产负债表）。
  ...
  若无法获取母公司单体资产负债表（如MCP工具仅返回合并报表）：
  (a) 尝试通过WebSearch/WebFetch获取年报PDF中的母公司单体报表
  (b) 若仍不可获取 → 使用合并口径，但必须标注：
...
1. **基础市场数据**（MCP工具优先）：
   ...
   - 无风险利率（Rf）：A股/港股标的使用中国十年期国债收益率；美股标的使用美国十年期国债收益率
   - 上市结构（H股/红筹/开曼/A股/美股）及适用税率
   - 确认用户持股渠道（直接/港股通/美股券商），若未指定则使用默认值
...
6. **历史价格数据**（MCP工具优先）：
   - 过去10年（或上市以来）周线收盘价，用于因子4步骤4股价位置评估
   - 记录数据覆盖区间

7. 所有数据来源记录URL/工具名称，汇总至报告末尾
...
- 步骤4使用Step 1获取的10年历史周线数据，计算当前股价分位与目标买入价分位
- 输出最终判断
...
*乌龟战法 AI Prompt v0.13 | 价值型 / 估值回归型选股 | 粗算精算分层+外推可信度评级+港股通美股支持+母公司负债分析+股价位置评估*
