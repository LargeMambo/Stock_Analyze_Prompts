# 乌龟战法：AI选股分析Prompt v0.8

> **版本说明**：v0.8 — 基于v0.7，主要变更：
> 1. 因子1B新增「模块〇：三大报表基础数据与关键指标」，在深度定性分析前系统性列出过去5年三大报表核心数据及关键财务比率（含利润表、盈利能力指标、增长率、资产负债表、现金流量表），要求对异常变动做出解释并锚定主营业务利润口径，为后续所有模块提供统一数据基础
> 2. 因子1B模块三（竞争格局与护城河）升级为两层递进结构：第一层「非技术性护城河」（商业壁垒，保留原5项）+ 第二层「技术性护城河」（数据与算法壁垒，新增4项：数据资产壁垒、核心算法/模型壁垒、履约/供应链系统壁垒、AI/前沿技术投入），并新增两层交互关系判断（是否构成复合护城河飞轮）
> 3. 因子1B汇总输出中护城河参数格式更新为 `[非技术] xxx + [技术] xxx`，新增 `复合护城河飞轮：[是/否]`
> 4. 因子4价值陷阱排查第2项细化为区分商业层收窄与技术层被颠覆
> 5. report_template同步更新（新增3.0节、3.3节两层结构）
...
> 目的：(1) 为后续八个模块提供统一数据基础；(2) 提前识别财务数据中的重大异常；(3) 锚定后续因子2/3应使用的利润口径。

**指令**：读取最近5年财务报表，按以下结构输出，所有计算展示公式。
...
必含项：现金及等价物、受限现金、短期投资、现金+短投合计、应收账款、流动资产合计、长期投资、商誉、无形资产、总资产、短期借款/债务、长期借款/债务、有息负债合计、预收/递延收入、总负债、股东权益

**(6) 资产负债表深度分析**

- **净现金/净负债**：逐年计算 = 现金+短投 - 有息负债，标注趋势方向
...
必含项：OCF、Capex、FCF、投资活动CF、融资活动CF、回购金额、OCF/营收、FCF/营收、Capex/营收
...
### 3.0 三大报表基础数据与关键指标
{(1)利润表核心数据 (2)关键盈利能力指标 (3)增长率分析 (4)重大变动说明 (5)资产负债表核心数据 (6)资产负债表深度分析（净现金/有息负债/资产周转率） (7)现金流量表核心数据}
...
本报告由AI模型基于乌龟战法v0.8框架和公开财务数据自动生成，仅供参考，不构成投资建议。投资者应自行核实所有数据，并根据自身风险承受能力做出投资决策。AI分析存在以下固有局限：(1) 财务数据可能存在提取误差；(2) 联网搜索的市场数据可能存在时滞；(3) 定性判断依赖有限信息；(4) 历史数据不代表未来表现。
```
...
*乌龟战法 AI Prompt v0.8 | 价值型 / 估值回归型选股*
