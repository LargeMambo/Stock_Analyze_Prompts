# 乌龟战法：AI选股分析Prompt v0.10

> **版本说明**：v0.10 — 基于v0.10，主要变更：
> 1. **因子1B新增模块九「控股折价分析」**：针对控股公司/多元化集团结构，新增SOTP估值、市值与主营业务隐含估值对比、折价分解、敏感性分析三个子模块
> 2. **因子1B汇总输出同步扩充**：新增模块九判断行及控股折价相关参数传递
> 3. **report_template同步更新**：新增3.9节（控股折价分析），原3.9重编号为3.10
> 4. **execution_workflow同步更新**：Step 3补充控股公司结构判断触发模块九的说明
>
> v0.10变更保留：
> - 模块〇所有数据表含「数据来源」列
> - 模块〇(8)因子2/3/4计算参数预提取
> - 现金口径扩展（广义/狭义）
> - 因子1B汇总输出含定量参数块
> - 模块三两层递进护城河结构
...
本报告由AI模型基于乌龟战法v0.10框架和公开财务数据自动生成，仅供参考，不构成投资建议。投资者应自行核实所有数据，并根据自身风险承受能力做出投资决策。AI分析存在以下固有局限：(1) 财务数据可能存在提取误差；(2) 联网搜索的市场数据可能存在时滞；(3) 定性判断依赖有限信息；(4) 历史数据不代表未来表现。
```
...
*乌龟战法 AI Prompt v0.10 | 价值型 / 估值回归型选股 | 控股折价分析增强版*
